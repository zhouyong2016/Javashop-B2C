...
;(function($, window, document, undefined){
    var addressSelect = function (ele, opts) {
        ...
        this.defaults = {
            'appApi': ctx + '/api/base/region/get-children.do?regionid=',
            'deData': null,
            'debug' : false,
            'names' : {
                province: 'province',
	...
	addressSelect.prototype = {
		...
		complete : function(){
			...
			for(var i = 0; i < appTabALen; i++) {
				var __html = appTabA.eq(i).find('em').html();
				_this.titleView += __html
			}
		...
		wordCount : function(index){
			...
			if(index > 1){
				var appTabA  = node.appBody.find('.app-address-tab-a'),
					fontSize = parseInt(node.appTitleView.css('fontSize')),
					_html    = '';
				for(var i = 0, len = appTabA.length; i < len; i++) {
					var __html = appTabA.eq(i).find('em').html();
					_html += __html
				}
		...
		setNameTag : function(region_grade, region_id, local_name){
			var region_grade = parseInt(region_grade);
		...
		complyDeApi : function(res){
			...
			// 设置默认tab
			_this.setNameTag(_this.index, _this.defaultArray[_this.index].re_id, _local_name);
		},
		...
		accessApi : function(region_id, region_grade){
			...
			var options = {
				url      : _this.options.appApi + region_id,
				type     : 'GET',
				dataType : 'JSON',
