var Prop={
	...
	initinputHtml:function(){
		...
		$.ajax({
			 type: "POST",
			 url: ctx+"/shop/admin/type/prop_input_item.html?ajax=yes",
			 dataType:'html',
...
var Param={
		...
		initInputHtml:function(){
			...
			$.ajax({
				 type: "get",
				 url: ctx+"/shop/admin/type/param-item.do",
				 dataType:'html',
				 success: function(html){ 
					self.inputHtml=html;
				 },
		...
		addGroupInput:function(){
			...
			group.find("td .delete").click(function(){$(this).parents("tr").remove();});
		},
...
var GoodsType=$.extend({
	...
	doDelete:function(){
		...
		this.deletePost(ctx+"/shop/admin/type/delete.do");
	...
	doRevert:function(){
		...
		this.deletePost(ctx+"/shop/admin/type/revert.do","选择的类型已被成功还原至类型列表中");		
	},
...
var GoodsField={
	...
	loadFieldList:function(typeid){
		var self = this;
		$("#field_box").load(ctx+'/shop/admin/field/list.do?ajax=yes&typeid='+typeid,function(){
			self.bindListEvent();
		});
	},
	...
	openAddDlg:function(){
		...
		$("#addFieldDlg").load(ctx+'/shop/admin/field/add.do?typeid='+self.typeid+'&ajax=yes',function(){
	...
	openEditDlg:function(fieldid){
		...
		$("#editFieldDlg").load(ctx+'/shop/admin/field/edit.do?typeid='+self.typeid+'&fieldid='+fieldid+'&ajax=yes',function(){
	...
	addField:function(){
		...
		var self= this;
		var options = {
				url :ctx+"/shop/admin/field/save-add.do?ajax=yes",
				type : "POST",
				dataType : 'json',
				success : function(result) {	
					$.Loading.hide();
	...
	editField:function(){
		...
		var self= this;
		var options = {
				url :ctx+"/shop/admin/field/save-edit.do?ajax=yes",
				type : "POST",
				dataType : 'json',
				success : function(result) {				
				 	if(result.result==1){
	...
	deleteField:function(fieldid){
		...
		var self= this;
		$.ajax({
			url:ctx+"/shop/admin/field/delete.do?fieldid="+fieldid+"&ajax=yes",
		    type:"post",
		    dataType:"json",
		    success:function(result){
			$.Loading.hide();
	...
	saveSort:function(){
		...
		var options = {
				url :ctx+"/shop/admin/field/saveSort.do?ajax=yes",
				type : "POST",
				dataType : 'json',
				success : function(result) {				
				 	if(result.result==1){
