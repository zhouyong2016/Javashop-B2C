var MemberPrice={
	...
	bindMbPricBtnEvent:function(){
		...
		$(".mempriceBtn").unbind("click").bind("click",function(){
			...
			if( parseFloat(price)!= price) {
				...
				$("#memberpricedlg").load('${ctx}/shop/admin/member-price/list.do?price='+price+'&ajax=yes',function(){
					self.initDlg();
				});
			}
		});		
	...
	createPriceItems:function(){
		...
		$("#levelPriceEditor>tbody>tr").each(function(){
			...
			member_price.push(lvPrice);
		});
		
		//同步会员specProp对象中的价格数组
		var propid = member_price_box.siblings(".ipt").attr("propid");
		SpecOper.syncMemberPrice(propid,member_price);
