...
var GoodsMultiSelector =  GoodsSelector;

//商品单个选择器
var GoodsSingleSelector={
	conid:undefined,
	init:function(conid,onConfirm)	{
		this.conid = conid;
		var self  = this;
	...
	search:function(onConfirm){
		var self = this;
		var options = {
				url :basePath+"selector!goods.do?ajax=yes&isSingle=1",
				type : "GET",
				dataType : 'html',
				success : function(result) {				
					$("#"+self.conid).empty().append(result);
	...
	getGoodsList:function(callback){
		var options = {
				url :basePath+"selector!listGoods.do?ajax=yes&isSingle=1",
				type : "GET",
				dataType : 'json',
				success : function(result) {				
					if(callback){
	...
	open:function(conid,onConfirm,catid){
		var self= this;
		var url = basePath+'selector!goods.do?ajax=yes&isSingle=1';
		if(catid){
			url=url+"&catid="+catid;
		}
