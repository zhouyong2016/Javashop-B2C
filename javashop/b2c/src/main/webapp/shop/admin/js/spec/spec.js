...
var Product= {
	...
	createEmptyTable:function(){
		...
		theadHtml+='<th style="width: auto;" id="priceTh">销售价</th>';
	...
	doCreatePros:function(specs,names,memPriceInput){
		...
		for(i in products){
			...
			var html ='<tr><td><input type="text"  name="sns"  style="width:150px"   autocomplete="off"><input type="hidden"  name="productids" ></td>';
			var specvids="";
			var specids ="";
			for(j in specar){
			...
			html+='<input type="text" class="price" name="prices" size="8" autocomplete="off" value="'+ price +'"><input type="button" class="mempriceBtn" value="会员价" /><div class="member_price_box" index="'+i+'">'+priceInputHtml+'</div></td>';
			...
			html+='<td><a href="javascript:;" ><img  class="delete" src='+ctx+'"/shop/admin/images/transparent.gif" productid="0"></a></td>';
			html+="</tr>";
			var trEl =$(html);
		...
		if(confirm("确定删除此货品吗？删除后不可恢复")){
			...
			if("0"!=productid){
				$.ajax({
					...
					success:function(result){
						if(result.result==1){
							if(confirm("此货品已经有客户购买，如果删除此订单将不能配货发货，请谨慎操作!\n点击确定删除此货品，点击取消保留此货品。")){
