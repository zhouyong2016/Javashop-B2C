 ...
 * @version v1.0
 * @since v6.0
 * 2016年11月29日 上午10:00:13 
 */

public class ActivityShipOrderFlowTest extends SpringTestSupport{
	...
	public void testActivity() throws Exception{
		...
		mockMvc.perform(
				...
				.param("activity_name", "免邮费的促销活动")
				.param("activity_type", "1")
				.param("bonus_id", "0")
				.param("description", "测试免邮费的促销活动")
				.param("endTime", "2017-12-31 10:07:09")
				.param("full_money", "30.00")
				.param("gift_id","0")
				.param("goods_ids", "273")
				.param("is_free_ship","1")
				.param("range_type", "2")
				.param("startTime", "2016-11-29 10:07:06")
				.session(session)
	...
	public void testBase() throws Exception{
		...
		MvcResult result = mockMvc.perform(
				...
				//订单价格32,
				.andExpect(MockMvcResultMatchers.jsonPath("$.data.order.order_amount").value(32.0))
				
				//商品价格为32,
				.andExpect(MockMvcResultMatchers.jsonPath("$.data.order.goods_amount").value(32.0))
				
				//促销活动免运费，原运费10元被减免,
				.andExpect(MockMvcResultMatchers.jsonPath("$.data.order.shipping_amount").value(0.0))
				...
				//发货
				mockMvc.perform(MockMvcRequestBuilders.post("/shop/admin/order-print/ship.do")
						.param("order_id", ""+orderid)
						.session(session)
						.contentType(MediaType.APPLICATION_JSON)
						.accept(MediaType.APPLICATION_JSON)
